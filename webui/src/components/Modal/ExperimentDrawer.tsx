...
class ExperimentDrawer extends React.Component<ExpDrawerProps, ExpDrawerState> {
    ...
    getExperimentContent = () => {
        axios
            .all([
                axios.get(`${MANAGER_IP}/experiment`,{headers:{'upstream': this.state.port, 'ExperimentID': this.props.ExperimentID}}),
                axios.get(`${MANAGER_IP}/trial-jobs`,{headers:{'upstream': this.state.port, 'ExperimentID': this.props.ExperimentID}}),
                axios.get(`${MANAGER_IP}/metric-data`,{headers:{'upstream': this.state.port, 'ExperimentID': this.props.ExperimentID}})
            ])
...
export default connect<any, any, any>((state,props)=>({port:state.PortReducer, ExperimentID: state.ExperimentReducer}) )(ExperimentDrawer);
