...
import { Row, Col, Tabs, Select, Icon } from 'antd';
...
class TrialsDetail extends React.Component<TrialsDetailProps, TrialDetailState> {
    ...
    getDetailSource = (ExperimentID ?: string) => {
        const HeaderExperimentID =  ExperimentID !== undefined ? ExperimentID : this.state.ExperimentID
        this.isOffIntervals();
        axios
            .all([
                axios.get(`${MANAGER_IP}/trial-jobs`, {headers:{'upstream': this.state.port, 'ExperimentID': HeaderExperimentID}}),
                axios.get(`${MANAGER_IP}/metric-data`, {headers:{'upstream': this.state.port, 'ExperimentID': HeaderExperimentID}})
            ])
    ...
    componentWillReceiveProps(nextProps: TrialsDetailProps) {
        ...
        if(nextProps.ExperimentID !== this.props.ExperimentID){
            this.setState({ExperimentID:nextProps.ExperimentID});
            this.getDetailSource(nextProps.ExperimentID);
        }
...
export default connect<any, any, any>((state,props)=>(
    ...
    ExperimentID: state.ExperimentReducer}) )(TrialsDetail);
